...
function calculateValidMoves(piece, currentSquare) {
    ...
    const kingInCheck = isKingInCheck(pieceColor);
...
function isKingInCheck(color) {
    const king = document.querySelector(`img.${color}.raja`);
    if (!king) return false;
    const kingSquare = king.closest('.square');
    return isSquareAttacked(kingSquare, color);
}


function simulateMoveAndCheck(piece, fromSquare, toSquare, color) {
    if (typeof fromSquare === "string") {
        fromSquare = document.getElementById(fromSquare);
    }
    if (typeof toSquare === "string") {
        toSquare = document.getElementById(toSquare);
    }

    if (!fromSquare || !toSquare) {
        console.error("simulateMoveAndCheck: invalid square(s)", fromSquare, toSquare);
        return false;
    }
    ...
    const king = document.querySelector(`.raja.${color}`);
    if (!king) return false;
    const kingSquare = king.closest('.column');
    const [_, kr, __, kc] = kingSquare.id.split('-');
    const kingRow = parseInt(kr);
    const kingCol = parseInt(kc);

    const inCheck = isSquareAttacked(kingRow, kingCol, color === "white" ? "black" : "white");

    originalParent.appendChild(piece);
    if (captured) toSquare.appendChild(captured);
